...
card_key_bp = Blueprint('card_key', __name__)
...
@card_key_bp.route('/generate', methods=['POST'])
...
@card_key_bp.route('/use', methods=['POST'])
def use_card_key():
    """使用卡密"""
    openid = session.get('openid')
    if not openid:
    ...
    try:
        ...
        if card_key.use_by(openid, user.nickname):
            ...
            return jsonify({
                'success': True,
                'message': f'卡密使用成功，获得{card_key.days}天体验时间',
        ...
        db.session.rollback()
        print(f"使用卡密失败: {e}")
        return jsonify({'error': '使用卡密失败'}), 500

@card_key_bp.route('/stats')
def get_stats():
    """获取卡密统计信息"""
    openid = session.get('openid')
    if not openid:
